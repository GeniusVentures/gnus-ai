	...
	for (const [networkName, provider] of networkProviders.entries()) {
		describe(`🔗 Chain: ${networkName}  Diamond: ${diamondName}`, function () {
			...
			it('Batch Transferring to two addresses', async () => {
				...
				const initBalance = await await geniusDiamond['balanceOf(address,uint256)'](
					owner,
					GNUS_TOKEN_ID,
				);
				...
				let balance = await await geniusDiamond['balanceOf(address,uint256)'](
					owner,
					GNUS_TOKEN_ID,
				);
				...
				balance = await await geniusDiamond['balanceOf(address,uint256)'](
					owner,
					GNUS_TOKEN_ID,
				);
			...
			it(`should verify the blocking and unblocking of transfers on ${networkName}`, async () => {
				...
				const balance = await await geniusDiamond['balanceOf(address)'](owner);
