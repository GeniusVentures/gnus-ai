...
after(async function () {
  ...
  try {
      await new Promise<void>((resolve, reject) => {
        try {
          MultichainGNUSERC20Tests.suite(networkDeployedInfo)
          // MultichainGNUSBridgeTests.suite(networkDeployedInfo);
          // MultichainErc20BatchTests.suite(networkDeployedInfo);
        } catch (error) {
          reject(error);
        }
      });
