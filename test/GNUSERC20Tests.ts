...
export function suite () {
  describe("GNUS ERC20 Hybrid Testing", async function () {
    ...
    it("Testing GNUS ERC20 interface is supported", async () => {
      ...
      assert(await gnusDiamond.supportsInterface(IERC20InterfaceID._hex), "Doesn't support IERC20Upgradeable");
    });
    ...
    it("Testing GNUS ERC20 transfer", async () => {
      ...
      const ownerSupply = await gnusDiamond["balanceOf(address)"](owner);
    ...
    it("Testing GNUS transferFrom & approval", async () => {
      ...
      await expect(gnusDiamond.transferFrom(addr3, owner, toWei(1))).to.eventually.be.rejectedWith(Error,
        /ERC20: insufficient allowance/);
      ...
      await expect(gnusDiamond.safeTransferFrom(addr3, escrowAIContractAddress, GNUS_TOKEN_ID, toWei(1), [])).to.eventually.be.rejectedWith(Error,
