...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 18-01-01 | 01 | 1 | SEC-09 | T-18-04 | 0.11.6 exit mechanics re-proven (probe-then-flip) BEFORE any gate spelling change — exactly-3 asserted via slither's own footer plus the three 07-03 site identities | probe (captured /tmp logs) | `slither --version \| grep -q "0.11.6" && grep -q ", 3 result(s) found" /tmp/18-01-slither06-bare.log && grep -q "GNUSWithdrawLimiterStorage.calculateCurrentBin" /tmp/18-01-slither06-bare.log && grep -q "GNUSBridge.approve(address,uint256)" /tmp/18-01-slither06-bare.log && grep -q "GNUSBridge.transferFrom(address,address,uint256)" /tmp/18-01-slither06-bare.log && grep -ci "unrecognized arguments\|not allowed with" /tmp/18-01-slither06-mutual.log` | n/a (probe logs) | ⬜ pending |
...
| 18-02-01 | 02 | 1 | SEC-10 | T-18-05 | Fixed rule present + valid in BOTH configs — a parse-broken rule must fail the check (verified: current broken rule exits 2); rule-shape greps scoped to the replaced rule block via a sed bounded extract | grep + semgrep validate | `sed -n '/id: unsafe-external-call/,/include: \["\*\*\/\*\.sol"\]/p' .semgrep.yml \| grep -q "languages: \[generic\]" && [ "$(sed -n '/id: unsafe-external-call/,/include: \["\*\*\/\*\.sol"\]/p' .semgrep.yml \| grep -c "pattern-not-inside")" -eq 4 ] && semgrep scan --validate --config .semgrep.yml && semgrep scan --validate --config .devcontainer/config/.semgrep.yml && ! grep -q "require(success" .semgrep.yml && ! grep -q "require(success" .devcontainer/config/.semgrep.yml && [ -z "$(git -C .devcontainer status --porcelain)" ]` | target files | ⬜ pending |
| 18-02-02 | 02 | 1 | SEC-10 | T-18-06 / T-18-07 / T-18-08 | Surface reaches the submodule with zero vendored/generated/test paths; D-11/D-12 exclusions applied and intact; rule proven to execute (positive control 2 → shipped 0) | probe (captured /tmp logs) | `grep -q -- "--no-git-ignore" package.json && [ "$(grep -o -- '--exclude ' package.json \| wc -l)" -ge 15 ] && grep -q -- "--exclude-rule diamond-selector-clash" package.json && grep -q -- "--exclude-rule hard-coded-secret" package.json && [ -s /tmp/18-02-shipped-rule.log ] && ! grep -qi "Rule parse error" /tmp/18-02-shipped-rule.log && ! grep -qE "lib/forge-std\|typechain-types/\|node_modules/\|(^\| )test/" /tmp/18-02-shipped-rule.log && grep -q "Findings: 2 (2 blocking)" /tmp/18-02-positive-control.log` | target + probe logs | ⬜ pending |
| 18-02-03 | 02 | 1 | SEC-10 | T-18-09 | Zero `: any`, zero top-level value imports (type-only-import constraint), tsc green, Hardhat suite at ledger baseline, semgrep reduced to exactly the 4 D-13-pending findings | regression (tsc + suite + scan) | `npx tsc --noEmit && [ "$(grep -c ': any' scripts/utils/GNUSLifecyclePolicyLinking.ts)" -eq 0 ] && [ "$(grep -cP '^import (?!type)' scripts/utils/GNUSLifecyclePolicyLinking.ts)" -eq 0 ] && yarn test > /tmp/18-02-hardhat-regression.log 2>&1; echo "exit=$?" \| grep -q "exit=0" && grep -q "666 passing" /tmp/18-02-hardhat-regression.log && yarn semgrep:scan > /tmp/18-02-post-typing.log 2>&1; [ "$(grep -c 'typescript-any-usage' /tmp/18-02-post-typing.log)" -eq 0 ] && grep -q "Findings: 4 (4 blocking)" /tmp/18-02-post-typing.log` | existing suite | ⬜ pending |
...
| 18-03-02 | 03 | 2 | SEC-10 | T-18-10 | D-13 exclusion applied per ruling; 3 consecutive zero-finding runs; all exclusions intact post-edit (missing stability logs fail the chain — `[ -s ]` guards) | probe (3× stability logs) | `grep -q -- "--exclude-rule unsafe-math-operation" package.json && for i in 1 2 3; do [ -s /tmp/18-03-semgrep-$i.log ] \|\| exit 1; grep -q "Findings: 0 (0 blocking)" /tmp/18-03-semgrep-$i.log \|\| exit 1; grep -qi "Rule parse error" /tmp/18-03-semgrep-$i.log && exit 1; done; for i in 1 2 3; do [ -s /tmp/18-03-semgrep-$i.log ] \|\| exit 1; grep -qE "lib/forge-std\|typechain-types/\|node_modules/\|(^\| )test/" /tmp/18-03-semgrep-$i.log && exit 1; done; echo "stability-logs-clean"` | target + probe logs | ⬜ pending |
...
Verify-chain quirks (18-01-01 / 18-02-01 / 18-02-02 / 18-02-03 / 18-03-02 — all empirically verified on this machine 2026-08-31, checker pass 2): negated greps and `&& exit 1` loops carry `[ -s <log> ]` existence guards because a missing log otherwise passes vacuously (grep rc 2 inverted by `!` / never triggering `&& exit 1`); exactly-N finding counts use the tools' own footers — slither `, 3 result(s) found` (leading comma load-bearing: bare `3 result(s) found` also matches `13 result(s) found`) and semgrep `Findings: 4 (4 blocking)` (semgrep's footer arrives on stderr, so every footer-grepped capture uses `> <log> 2>&1`; verified 2026-08-31) — because per-finding name counting does not track finding counts (slither repeats detector names in reference URLs; semgrep groups same-rule findings per file under one rule-name header); rule-shape greps use a sed bounded extract from the rule id to its own `include:` line (`languages: [generic]` sits ~25 lines below the id, and the file carries 8 pre-existing `pattern-not-inside` in other rules); the clean-tree check uses `[ -z "$(...)" ]` because BSD wc pads stdin counts.
...
**Approval:** ready — 2026-08-31 (revised per checker findings pass 1 + verify-chain hardening per checker pass 2; per-task map byte-mirrors the plans' automated verifies)
