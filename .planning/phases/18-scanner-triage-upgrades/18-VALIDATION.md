...
| 18-02-02 | 02 | 1 | SEC-10 | T-18-06 / T-18-07 / T-18-08 | Surface reaches the submodule with zero vendored/generated/test paths; D-11/D-12 exclusions applied and intact; rule proven to execute (positive control 2 → shipped 0) | probe (captured /tmp logs) | `grep -q -- "--no-git-ignore" package.json && [ "$(grep -o -- '--exclude ' package.json \| wc -l)" -ge 15 ] && grep -q -- "--exclude-rule diamond-selector-clash" package.json && grep -q -- "--exclude-rule hard-coded-secret" package.json && [ -s /tmp/18-02-shipped-rule.log ] && ! grep -qi "Rule parse error" /tmp/18-02-shipped-rule.log && ! grep -qE "lib/forge-std\|typechain-types/\|node_modules/\|(^\| )test/" /tmp/18-02-shipped-rule.log` | target + probe logs | ⬜ pending |
...
| 18-03-02 | 03 | 2 | SEC-10 | T-18-10 | D-13 exclusion applied per ruling; 3 consecutive zero-finding runs; all exclusions intact post-edit (missing stability logs fail the chain — `[ -s ]` guards) | probe (3× stability logs) | `grep -q -- "--exclude-rule unsafe-math-operation" package.json && for i in 1 2 3; do [ -s /tmp/18-03-semgrep-$i.log ] \|\| exit 1; grep -qi "Rule parse error" /tmp/18-03-semgrep-$i.log && exit 1; done; for i in 1 2 3; do [ -s /tmp/18-03-semgrep-$i.log ] \|\| exit 1; grep -qE "lib/forge-std\|typechain-types/\|node_modules/" /tmp/18-03-semgrep-$i.log && exit 1; done; echo "stability-logs-clean"` | target + probe logs | ⬜ pending |
...
Verify-chain quirks (18-01-01 / 18-02-01 / 18-02-02 / 18-02-03 / 18-03-02 — all empirically verified on this machine 2026-08-31, checker pass 2): negated greps and `&& exit 1` loops carry `[ -s <log> ]` existence guards because a missing log otherwise passes vacuously (grep rc 2 inverted by `!` / never triggering `&& exit 1`); exactly-N finding counts use the tools' own footers — slither `, 3 result(s) found` (leading comma load-bearing: bare `3 result(s) found` also matches `13 result(s) found`) and semgrep `Findings: 4 (4 blocking)` — because per-finding name counting does not track finding counts (slither repeats detector names in reference URLs; semgrep groups same-rule findings per file under one rule-name header); rule-shape greps use a sed bounded extract from the rule id to its own `include:` line (`languages: [generic]` sits ~25 lines below the id, and the file carries 8 pre-existing `pattern-not-inside` in other rules); the clean-tree check uses `[ -z "$(...)" ]` because BSD wc pads stdin counts.
