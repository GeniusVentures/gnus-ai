...
**Current focus:** Phase 18 — scanner triage upgrades
**Progress:** [██████████] 100%
...
### Phase 7 Decisions Logged (07-post-review — WR-01 closure, 2026-08-28)

- Owner ordered the WR-01 root-cause fix (dependency refresh) ahead of mainnet. All 23 medium+ snyk findings lived in the root project only (.devcontainer, .devcontainer/config, lib/forge-std manifests clean), in 7 transitive packages, each with a fixed version inside the parent's existing range: tar@7.5.2 (12, via node-gyp), minimatch@10.1.1 (3, via glob@11/13 ≤ cacache), ws@8.18.3 (2, via web3-providers-ws), ip-address@10.1.0 (2, via socks), picomatch@4.0.3 (2, via tinyglobby/ts-declaration-location), diff@4.0.2 (1, via ts-node), @isaacs/brace-expansion@5.0.0 (1 Critical, via minimatch@10).
- Mechanism: range-qualified `resolutions` in package.json (8 keys) — re-points ONLY the matching descriptors, zero direct dependencies changed, every other pin frozen. minimatch resolved to 10.2.6 (already in-tree via eslint ranges → dedupes instead of adding a copy); ethers' exact-pinned ws@8.17.1/8.18.0 deliberately NOT overridden (lockstep pins; not snyk-flagged paths). Lockfile net -63/+38 lines. `keccak`/`secp256k1` rebuild triggers were normal tree-change rebuilds.
- Gate state after: `yarn snyk:test` exit 0 ("Tested 4 projects, no vulnerable paths"); `yarn npm audit --severity moderate` exit 0. The CI snyk step is now a zero-finding hard gate — red means a newly published advisory, nothing recorded (workflow header comment updated to say so). Follow-up 3(a)'s snyk half is CLOSED; the OSV 115-advisory set remains an open owner item (not a CI gate; its tar@7.5.2 share is fixed by this change but axios/undici/handlebars/fast-xml-parser entries are untouched by scope).
- Regression gates: Hardhat 665/2/1 (exact baseline, sole failure = known GNUSControlStorage chainID); `yarn install --immutable --check-cache` exit 0. Foundry wrapper 214/3/3 — the third failure is the 07-04-recorded flaky `AccessControlInvariant` ("User3 should not have UPGRADER_ROLE"), reproduced 3/3 runs tonight INCLUDING once on the pre-resolution tree via stash isolation (resolutions exonerated; no .sol, no direct dep changed). The flake is reproducing consistently tonight where 07-04 saw it clear on re-run — root fix unchanged (seed invariant config / align invariant with handler grant surface), belonging to the Foundry suite's owning phase; not a WR-01 blocker. Bare `forge test` without the wrapper node is NOT a valid gate (26+ "Diamond has no code" setUp artifacts — DiamondDeployment.sol constants point at a wrapper-deployed localhost diamond).
