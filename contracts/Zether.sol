...
contract Zether is Initializable, GeniusAccessControl {
    ...
    function init(address _token, uint256 _epochLength)
        external
        onlySuperAdminRole
    {
        ...
        ZSCStorage.layout().pending[keccak256(abi.encode(empty))][1] = Utils
            .g(); // "register" the empty account...
        // make sure there
        libEncryption.init();
    }
        ...
        returns (Utils.G1Point[2][] memory accounts)
    {
        ...
        for (uint256 i = 0; i < size; i++) {
            ...
            if (ZSCStorage.layout().lastRollOver[yHash] < epoch) {
                Utils.G1Point[2] memory scratch = ZSCStorage.layout().pending[
                    yHash
                ];
    ...
    function rollOver(bytes32 yHash) internal {
        ...
        if (ZSCStorage.layout().lastRollOver[yHash] < e) {
            ...
            ZSCStorage.layout().acc[yHash][0] = scratch[0][0].add(
                scratch[1][0]
            );
            ZSCStorage.layout().acc[yHash][1] = scratch[0][1].add(
                scratch[1][1]
            );
    ...
    function register(
        ...
        uint256 challenge = uint256(keccak256(abi.encode(address(this), y, K)))
            .mod();
    ...
    function zTransfer(
        ...
        require(
            registered(beneficiaryHash),
            "Miner's account is not yet registered."
        ); // necessary so that receiving a fee can't "backdoor" you into registration.
        rollOver(beneficiaryHash);
        Utils.G1Point memory beneficiaryPending = ZSCStorage.layout().pending[
            beneficiaryHash
        ][0];
        ZSCStorage.layout().pending[beneficiaryHash][0] = beneficiaryPending
            .add(Utils.g().mul(ZSCStorage.layout().fee));
        ...
        for (uint256 i = 0; i < ZSCStorage.layout().nonceSet.length; i++) {
            require(
                ZSCStorage.layout().nonceSet[i] != uHash,
                "Nonce already seen!"
            );
        }
    ...
    function zWithdraw(
        ...
        require(
            0 <= bTransfer && bTransfer <= MAX,
            "Transfer amount out of range."
        );
        Utils.G1Point[2] memory scratch = ZSCStorage.layout().pending[yHash];
        ZSCStorage.layout().pending[yHash][0] = scratch[0].add(
            Utils.g().mul(bTransfer.neg())
        );
    ...
    {
        bytes32 yHash = keccak256(abi.encode(y));
        uint256 plainValue = libEncryption.decryptWithSavedData(
            privateKey,
            yHash
        );
