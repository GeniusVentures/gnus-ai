...
export function createRPCConfig(options: BaseRPCOptions): RPCDiamondDeployerConfig {
	...
	if (options.useHardhatConfig !== false && !options.rpcUrl) {
		if (options.verbose) {
			console.log(
				chalk.blue('📋 Using hardhat configuration for diamond and network settings'),
			);
		}
		...
		try {
			...
			console.error(
				chalk.red(`❌ Failed to create config from hardhat: ${(error as Error).message}`),
			);
			console.error(
				chalk.yellow(
					'💡 Try using legacy mode with --no-use-hardhat-config and explicit --rpc-url',
				),
			);
			process.exit(1);
		}
...
export async function showPreOperationInfo(
	...
	console.log(
		`⛽ Gas Limit Multiplier: ${chalk.white(config.gasLimitMultiplier || '1.2')}`,
	);
