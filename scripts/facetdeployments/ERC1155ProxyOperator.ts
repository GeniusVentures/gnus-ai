...
const afterDeploy: AfterDeployInit = async (networkDeployInfo: INetworkDeployInfo) => {
  ...
  const gnusERC1155ProxyOperator = gnusDiamond as ERC1155ProxyOperator;
  const NFTProxyRole = await gnusERC1155ProxyOperator.NFT_PROXY_OPERATOR_ROLE({
    gasLimit: 600000,
  });
  ...
  if (networkName in OpenSeaProxyAddresses) {
    ...
    try {
      await gnusERC1155ProxyOperator.grantRole(NFTProxyRole, proxyAddress, {
        gasLimit: 600000,
      });
    } catch (e) {
      debuglog(
        `Warning, couldn't grant proxy role for ${networkName} OpenSea NFT contract at ${proxyAddress}\n${e}`,
      );
    }
  }
};
