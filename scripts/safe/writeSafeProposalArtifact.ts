 ...
 */
export function writeSafeProposalArtifact(params: WriteSafeProposalArtifactParams): string {
	...
	if (!ethers.isAddress(params.target)) {
		throw new Error(`writeSafeProposalArtifact: invalid 'target': ${params.target}`);
	}
	if (!/^0x[0-9a-fA-F]*$/.test(params.calldata)) {
		throw new Error("writeSafeProposalArtifact: 'calldata' must be 0x-prefixed hex");
	}
