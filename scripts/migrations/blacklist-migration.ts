...
export class BlacklistMigration {
	...
	private async isLegacyBlacklisted(address: string): Promise<boolean> {
		...
		console.warn(
			`Legacy blacklist check not implemented for ${address}. Please provide known blacklisted addresses manually.`,
		);
		return false;
	} /**
	...
	async runMigration(knownBlacklistedAddresses?: string[]): Promise<void> {
		...
		try {
			...
			const addresses =
				knownBlacklistedAddresses || (await this.getLegacyBlacklistedAddresses());

			if (addresses.length === 0) {
				console.log('No addresses to migrate');
				console.log(
					'Please provide an array of known blacklisted addresses as parameter to runMigration()',
				);
				return;
			}
	...
	printSummary(): void {
		...
		console.log(
			`Legacy blacklisted: ${this.migrationLog.filter((entry) => entry.wasLegacyBlacklisted).length}`,
		);
		console.log(
			`Successfully migrated: ${this.migrationLog.filter((entry) => entry.migrated).length}`,
		);
	...
	async validateMigration(addressesToCheck?: string[]): Promise<boolean> {
		...
		for (const address of addresses) {
			try {
				...
				if (isLegacy && !isV2) {
					console.error(
						`Validation failed for ${address}: Legacy blacklisted but not in V2`,
					);
...
async function main() {
	...
	try {
		...
		console.log(
			'2. Add known blacklisted addresses to the knownBlacklistedAddresses array above',
		);
		console.log('3. Uncomment and configure the diamond contract loading code below');
		console.log(
			'4. Run with: npx ts-node scripts/migrations/blacklist-migration.ts [--dry-run]',
		);
